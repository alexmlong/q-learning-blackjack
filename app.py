...
class BlackjackAgent:
    ...
    def update(
        ...
        # update q table for each state in history with discount value
        # so need the reward value and that's it?
        discounted_rewards = self.calc_discounted_rewards(reward)
        for index, step in enumerate(reversed(self.history)):
            # discount proportion is 0% for first. if there were 10 items then pretend like 11 and discount proprortionally. so first gets 0%, then 9%, then 18%
